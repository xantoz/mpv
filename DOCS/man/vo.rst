...
``drm`` (Direct Rendering Manager)
    ...
    ``--drm-render-plane=<primary|overlay|N>``
        Select the DRM plane to which video and OSD is rendered to, under normal
    ...
    ``--drm-drmprime-video-plane=<primary|overlay|N>``
        ...
        accepts the same values as ``--drm-render-plane``. (default: overlay)

        To be able to successfully play 4K video on various SoCs you might need
        to set ``--drm-render-plane=overlay --drm-drmprime-video-plane-id=primary``
        and setting ``--drm-render-size=1920x1080``, to render the OSD at a
