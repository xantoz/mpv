...
struct drm_pflip_cb_closure {
    ...
    struct drm_vsync_tuple *vsync; // vsync tuple of the latest page flip. page_flipped updates this
    struct vo_vsync_info *vsync_info; // where the page_flipped routine writes its output
    bool *waiting_for_flip; // page_flipped writes false here before returning
};
...
void page_flipped(int fd, unsigned int msc, unsigned int sec,
