...
struct drm_atomic_context *drm_atomic_create_context(struct mp_log *log, int fd, int crtc_id,
                                                     int connector_id,
                                                     int render_plane_idx, int drmprime_video_plane_idx)
{
    ...
    for (unsigned int j = 0; j < plane_res->count_planes; j++) {
        ...
        if (possible_crtcs & (1 << crtc_index)) {
            ...
            if (value != DRM_PLANE_TYPE_CURSOR) { // Skip cursor planes
                ...
                if (layercount == render_plane_idx) {
                    ctx->render_plane = plane;
                    continue;
                }
    ...
    // render plane was specified as either of the special options: any primary plane or any overlay plane
    if (!ctx->render_plane) {
        const int render_plane_id = (render_plane_idx == DRM_OPTS_OVERLAY_PLANE) ? overlay_id : primary_id;
        const char *plane_type = (render_plane_idx == DRM_OPTS_OVERLAY_PLANE) ? "overlay" : "primary";
        if (render_plane_id) {
            mp_verbose(log, "Using %s plane %d as render plane\n", plane_type, render_plane_id);
            ctx->render_plane = drm_object_create(log, ctx->fd, render_plane_id, DRM_MODE_OBJECT_PLANE);
        } else {
            mp_err(log, "Failed to find render plane with idx=%d\n", render_plane_idx);
            goto fail;
        }
    } else {
        mp_verbose(log, "Found render plane with ID %d\n", ctx->render_plane->id);
    }
...
void drm_atomic_destroy_context(struct drm_atomic_context *ctx)
{
    ...
    drm_object_free(ctx->render_plane);
...
bool drm_atomic_save_old_state(struct drm_atomic_context *ctx)
{
    ...
    if (!drm_atomic_save_plane_state(ctx->render_plane, &ctx->old_state.render_plane))
        ret = false;
...
bool drm_atomic_restore_old_state(drmModeAtomicReqPtr request, struct drm_atomic_context *ctx)
{
    ...
    if (!drm_atomic_restore_plane_state(request, ctx->render_plane, &ctx->old_state.render_plane))
        ret = false;
