...
struct drm_mode_spec {
    enum {
        ...
        DRM_MODE_SPEC_BY_NUMBERS, // Specified by width, height and optionally refresh
...
static bool all_digits(const char *str)
{
    ...
    for (const char *c = str; *c != '\0'; ++c) {
        all_digits = all_digits && mp_isdigit(*c);
    }
    return all_digits;
}
                       ...
                       double refresh)
{
    if (refresh == 0.0) { // Exact compare is fine, because we check whether or not refresh has been set
        return
            (mode->hdisplay == height) &&
            (mode->vdisplay == width);
    } else {
        double mode_refresh = mode_get_Hz(mode);
        return
            (mode->hdisplay == height) &&
            (mode->vdisplay == width) &&
            (fabs(refresh - mode_refresh) < 0.001);
    }
}
...
static bool setup_mode_highest(struct kms *kms)
{
    ...
    for (unsigned int i = 0; i < kms->connector->count_modes; ++i) {
        ...
        const int current_area = current_mode->hdisplay * current_mode->vdisplay;
...
static void kms_show_connector_name_and_state_callback(struct mp_log *log, int card_no,
...
static void kms_show_foreach_card(struct mp_log *log, void (*show_fn)(struct mp_log*,int))
{
