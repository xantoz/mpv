...
const struct m_sub_options drm_conf = {
    .opts = (const struct m_option[]) {
        ...
        OPT_CHOICE("drm-atomic", drm_atomic, 0,
                   ({"no", 0},
                    {"auto", 1})),
        OPT_CHOICE_OR_INT("drm-render-plane", drm_render_plane, 0, 0, INT_MAX,
        ...
        OPT_SIZE_BOX("drm-render-size", drm_render_size, 0),
        {0},
    },
    .defaults = &(const struct drm_opts) {
        .drm_mode_spec = "preferred",
        .drm_atomic = 1,
        .drm_render_plane = DRM_OPTS_PRIMARY_PLANE,
    ...
    *kms = (struct kms) {
    ...
    if (!use_atomic) {
        ...
        kms->atomic_context = drm_atomic_create_context(kms->log, kms->fd, kms->crtc_id,
                                                        kms->connector->connector_id,
                                                        render_plane, drmprime_video_plane);
