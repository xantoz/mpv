...
struct priv {
    ...
    struct mpv_opengl_drm_render_size render_size;
};
...
static bool init_gbm(struct ra_ctx *ctx)
{
    ...
    MP_VERBOSE(ctx->vo, "Initializing GBM surface (%d x %d)\n",
        p->render_size.width, p->render_size.height);
    p->gbm.surface = gbm_surface_create(
        p->gbm.device,
        p->render_size.width,
        p->render_size.height,
        p->gbm_format,
...
static bool crtc_setup_atomic(struct ra_ctx *ctx)
{
    ...
    drm_object_set_property(request, atomic_ctx->render_plane, "FB_ID", p->fb->id);
    drm_object_set_property(request, atomic_ctx->render_plane, "CRTC_ID", p->kms->crtc_id);
    drm_object_set_property(request, atomic_ctx->render_plane, "SRC_X",   0);
    drm_object_set_property(request, atomic_ctx->render_plane, "SRC_Y",   0);
    drm_object_set_property(request, atomic_ctx->render_plane, "SRC_W",   p->render_size.width << 16);
    drm_object_set_property(request, atomic_ctx->render_plane, "SRC_H",   p->render_size.height << 16);
    drm_object_set_property(request, atomic_ctx->render_plane, "CRTC_X",  0);
    drm_object_set_property(request, atomic_ctx->render_plane, "CRTC_Y",  0);
    drm_object_set_property(request, atomic_ctx->render_plane, "CRTC_W",  p->kms->mode.mode.hdisplay);
    drm_object_set_property(request, atomic_ctx->render_plane, "CRTC_H",  p->kms->mode.mode.vdisplay);
...
static void drm_egl_swap_buffers(struct ra_ctx *ctx)
{
    ...
    if (atomic_ctx) {
        drm_object_set_property(atomic_ctx->request, atomic_ctx->render_plane, "FB_ID", p->fb->id);
        drm_object_set_property(atomic_ctx->request, atomic_ctx->render_plane, "CRTC_ID", atomic_ctx->crtc->id);
        drm_object_set_property(atomic_ctx->request, atomic_ctx->render_plane, "ZPOS", 1);
...
// If the render plane supports ARGB we want to use that, but if it doesn't we
...
static bool probe_gbm_format(struct ra_ctx *ctx, uint32_t argb_format, uint32_t xrgb_format)
{
    ...
    if (!p->kms->atomic_context) {
        ...
        MP_VERBOSE(ctx->vo, "Not using DRM Atomic: Use %s for render plane.\n",
                   gbm_format_to_string(xrgb_format));
        return true;
    }

    drmModePlane *drmplane =
        drmModeGetPlane(p->kms->fd, p->kms->atomic_context->render_plane->id);
    ...
    if (have_argb) {
        ...
        MP_VERBOSE(ctx->vo, "%s supported by render plane.\n", gbm_format_to_string(argb_format));
        result = true;
    } else if (have_xrgb) {
        p->gbm_format = xrgb_format;
        MP_VERBOSE(ctx->vo, "%s not supported by render plane: Falling back to %s.\n",
...
static bool drm_egl_init(struct ra_ctx *ctx)
{
    ...
    p->kms = kms_create(ctx->log, ctx->vo->opts->drm_opts->drm_connector_spec,
                        ...
                        ctx->vo->opts->drm_opts->drm_render_plane,
    ...
    if (ctx->vo->opts->drm_opts->drm_render_size.wh_valid) {
        if (p->kms->atomic_context) {
            p->render_size.width = ctx->vo->opts->drm_opts->drm_render_size.w;
            p->render_size.height = ctx->vo->opts->drm_opts->drm_render_size.h;
        } else {
            p->render_size.width = p->kms->mode.mode.hdisplay;
            p->render_size.height = p->kms->mode.mode.vdisplay;
            MP_WARN(ctx, "Setting render plane size is only available with DRM atomic, defaulting to screen resolution\n");
        }
    } else {
        p->render_size.width = p->kms->mode.mode.hdisplay;
        p->render_size.height = p->kms->mode.mode.vdisplay;
    ...
    ra_add_native_resource(ctx->ra, "drm_render_size", &p->render_size);
