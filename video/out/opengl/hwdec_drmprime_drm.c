                         ...
                         struct mp_rect *src, struct mp_rect *dst, bool newframe)
{
    ...
    if (hw_image) {
        ...
        struct mpv_opengl_drm_render_size *render_size = ra_get_native_resource(hw->ra, "drm_render_size");
        if (render_size) {
            scale_dst_rect(hw, render_size->width, render_size->height, dst, &p->dst);
        } else {
            p->dst = *dst;
        }
        p->src = *src;
...
{
    struct priv *p = hw->priv;
    int render_plane, drmprime_video_plane;

    p->log = hw->log;

    void *tmp = talloc_new(NULL);
    struct drm_opts *opts = mp_get_config_group(tmp, hw->global, &drm_conf);
    render_plane = opts->drm_render_plane;
    ...
    if (drm_params) {
        p->ctx = drm_atomic_create_context(p->log, drm_params->fd, drm_params->crtc_id,
                                           drm_params->connector_id, render_plane, drmprime_video_plane);
        if (!p->ctx) {
